...
# 500,000 to 1,000,000 for thorough scrape.
def get_anime_from_mal(cooldown=1, timeout=1, id=0, debug=False) -> Anime:
    time.sleep(cooldown);
    ...
    while is_generating:
        if content.status_code == 200 and "data" in json_content:
            ...
            url = json_content["data"]["images"]["jpg"]["large_image_url"];
...
# Multiple animes generated per page and 88 known pages.
def get_anime_from_gogo(page=1, n_episodes=2700, debug=False, progress_bar=None) -> List[Anime]:
    animes = [];
    ...
    if content.status_code == 200:
        ...
        for tag in tags:
            ...
            image_url = re.findall(r'src\s*=\s*"([^"]*)"', str(html.find_all(class_="anime_info_body_bg")));
            title = re.search(r'<h1>(.*?)</h1>', str(html.find_all(class_="anime_info_body_bg")));
            title = title.group(1);
...
def get_all_anime(total_pages=88, total_urls=1000000) -> List[Anime]:
    # Benchmark for Mal is 17.5 days | 75 days: 100 threads | cf 2 - 6 hours : 100 threads.
    animes = [];

    def chunk(start: int = None, end: int = None):
        for url in tqdm(range(start, end)):
            anime = get_anime_from_mal(id=url);
            try:
                x = anime.title;
                animes.append(anime);
            except:
    ...
    def chunk(start: int = None, end: int = None):
        for page in tqdm(range(start, end)):
            lst = get_anime_from_gogo(page=page);
...
def to_json(animes = []) -> Anime:
    """ Save anime array to json. """
...
"""
Please note this script was built a long time ago and the urls may not work or
the site structures may have changed.
